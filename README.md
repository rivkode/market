...
<a id="concurrency-test"></a>
# 동시성 테스트 결과 (300건)

`StockFacadeTest`에서 동일한 재고(`quantity=300`)에 대해 300개 요청을 동시에 발생시켜,
락 전략별 재고 차감 결과와 처리 시간을 비교했습니다.

- 테스트 코드: [StockFacadeTest.java](src/test/java/sample/market/application/product/stock/StockFacadeTest.java)
- 실행 방식: `ExecutorService(100 threads)` + `CountDownLatch(300)`
- 검증 조건: 모든 요청 완료 후 재고가 `0`인지 검증

## 실측 결과

| 구분 | 처리 시간 |
|---|---:|
| 비관적 락 (Pessimistic Lock) | 1.51초 |
| 낙관적 락 (Optimistic Lock) | 16.61초 |

- 동일 조건에서 비관적 락이 낙관적 락 대비 약 11배 빠르게 측정되었습니다.
...
- 구현 포인트:
  - `StockRepository`에서 `@Lock(LockModeType.PESSIMISTIC_WRITE)` 사용
  - DB 레벨에서 쓰기 락을 먼저 점유하여 충돌을 직렬화
...
- 구현 포인트:
  - `Stock` 엔티티의 `@Version` 필드 기반 충돌 감지
  - 충돌 발생 시 재시도(backoff) 로직 수행
  - `StockFacade`의 재시도 루프(`maxRetries=20`, sleep 50ms 지수 백오프) + `StockServiceImpl` 내부 재시도 대기(`Thread.sleep(100)`)로 인해 고경합 상황에서 누적 지연이 커짐

## 해석

- 본 시나리오처럼 동일 row에 대한 고경합(300건 동시 갱신)에서는,
  충돌 이후 재시도 비용이 큰 낙관적 락보다 비관적 락이 더 유리했습니다.
- 반대로, 충돌 가능성이 낮고 읽기 비중이 높은 트래픽에서는 낙관적 락이 유리할 수 있어,
  트래픽 특성에 따라 락 전략을 분리 적용하는 것이 적절합니다.

## 재현 방법

사전 준비:
- Redis/MySQL 실행 필요 (`docker-compose.yml` 기준)
